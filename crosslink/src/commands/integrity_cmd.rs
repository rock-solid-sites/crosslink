...
use crate::hydration::{
    hydrate_from_state, hydrate_v2_safely_locked, V2HydrateOutcome,
};
...
fn check_hydration(
    ...
    // gh#125 r3 (R3): the locked re-check + clear + hydrate must run under
    // ONE hub-write-lock acquisition. The lock is NON-REENTRANT (file-based
    // create_new + 30s timeout): holding it here and then calling
    // hydrate_v2_safely (which acquires again) would double-acquire — 30s
    // wait -> Err -> Skipped, so the repair would report Fail EVERY time, not
    // only on a genuine decision flip. Ordering per the amended plan:
    // re-emit (above) -> locked re-check -> clear -> hydrate. The locked
    // re-check runs BEFORE clear so a flip in the window (e.g. a concurrent
    // v3 join) refuses without ever leaving an empty-but-recoverable DB
    // (audit NIT #4).
    let lock_path = cache_dir.join(".hub-write-lock");
    let lock = match crate::sync::acquire_hub_lock(&lock_path) {
        Ok(lock) => lock,
        Err(e) => {
            // Lock acquisition failure is uncertainty — refuse WITHOUT
            // clearing; the snapshot taken above is the recovery handle.
            return Ok(CheckResult {
                name: "hydration".to_string(),
                status: CheckStatus::Fail(format!(
                    "{summary}; could not acquire the hub write lock for the v2 repair: {e}. \
                     Snapshot at {snapshot_rel}. Re-run the check once the lock frees."
                )),
            });
        }
    };

    // Locked re-check: the authoritative decision under the hub write lock.
    // If it flipped since the unlocked pre-check above, refuse before the
    // clear so the destructive path never runs on a stale projection.
    let v2_decision_locked = crate::hub_v3::v2_file_path_decision(
        &cache_dir,
        &crate::sync::read_tracker_remote(crosslink_dir),
    );
    if !v2_decision_locked.is_allowed() {
        let reason = v2_decision_locked
            .reason()
            .unwrap_or("unknown fail-closed reason");
        return Ok(CheckResult {
            name: "hydration".to_string(),
    ...
    let stats = match hydrate_v2_safely_locked(crosslink_dir, &cache_dir, db, &lock)? {
        V2HydrateOutcome::Hydrated(stats) => stats,
        V2HydrateOutcome::Skipped { reason } => {
            // The locked re-check above was Allowed but the dispatcher's
            // re-probe skipped (should not happen — both consult the same
            // refs under the same lock — but fail closed rather than report
            // a bogus repair).
            return Ok(CheckResult {
                name: "hydration".to_string(),
                status: CheckStatus::Fail(format!(
